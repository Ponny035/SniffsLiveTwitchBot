...
async def subscription_payout(username, sub_month_count, usernames, send_message, send_message_feed):
    ...
    await send_message_feed(f"@{username} ได้รับ {sub_to_point} sniffscoin และผู้ชมทั้งหมด {len(usernames)} คนได้รับ 1 sniffscoin")
    subscription_payout_feed(username, sub_to_point, len(usernames))
...
async def gift_subscription_payout(username, recipent, usernames, send_message):
    ...
    gift_subscription_payout_feed(username, recipent, sub_to_point, len(usernames))
...
async def anongift_subscription_payout(recipent, usernames, send_message, send_message_feed):
    add_coin(recipent, sub_to_point, True)
    payday(usernames, 1, True)
    await send_message("ขอบคุณ Gift จากผู้ไม่ประสงค์ออกนามค่าา sniffsHeart sniffsHeart sniffsHeart")
    await send_message_feed(f"@{recipent} ได้รับ {sub_to_point} sniffscoin และผู้ชมทั้งหมด {len(usernames)} คนได้รับ 1 sniffscoin")
    anongift_subscription_payout_feed(recipent, sub_to_point, len(usernames))
